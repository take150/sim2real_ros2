...
class GaussianModel(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device):
        ...
        # self.rnn_other = nn.RNN(input_size=13, hidden_size=64, num_layers=1, batch_first=True)
    ...
    def compute(self, inputs, role=""):
        ...
        # features_other = self.features_extractor_joints_other_container(out_other[:, -1, :])
        # features_other = self.features_extractor_joints_other_container(torch.cat([states['joint_other'], states['actions_other']], dim=-1))
        # output = self.net_container_2(torch.cat([features, states['z_actor']], dim=1))
        # output = self.net_container_2(torch.cat([features, features_other], dim=-1))
...
models = {}
models["robot_1"] = {}
# models["value"] = DeterministicModel(env.state_space("robot_1"), env.action_spaces["robot_1"], device)
