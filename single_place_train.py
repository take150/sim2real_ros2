...
class GaussianModel(GaussianMixin, Model):
    def __init__(self, observation_space, action_space, device):
        ...
        self.rnn_grasp = nn.RNN(input_size=13, hidden_size=128, num_layers=1, batch_first=True)
        self.net_container_grasp = nn.Sequential(
    ...
    def compute(self, inputs, role=""):
        ...
        if True:
            out, _ = self.rnn_grasp(torch.cat([states['joint'], states['actions']], dim=-1))
            output = self.net_container_grasp(torch.cat([out[:, -1, :], states['object'][:, -1, :]], dim=-1))
            output = self.policy_layer(output)
        elif True:
            out, _ = self.rnn_move(torch.cat([states['joint'], states['actions']], dim=-1))
            output = self.net_container_move(torch.cat([out[:, -1, :], states['object'][:, -1, :], states['goal'][:, -1, :]], dim=-1))
            output = self.policy_layer(output)
        elif True:
            out, _ = self.rnn_place(torch.cat([states['joint'], states['actions']], dim=-1))
            output = self.net_container_place(torch.cat([out[:, -1, :], states['object'][:, -1, :], states['goal'][:, -1, :]], dim=-1))
            output = self.policy_layer(output)
...
class DeterministicModel(DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device):
        ...
        self.rnn = nn.RNN(input_size=41, hidden_size=256, num_layers=2, batch_first=True)
    ...
    def compute(self, inputs, role=""):
        ...
        out, _ = self.rnn(torch.cat([states['joint'], states['actions'], states['object'], states['goal']], dim=-1))
